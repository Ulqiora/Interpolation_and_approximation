...
IGraphByDiscrete2D *CubicSplines::CreateFunction(const std::vector<double> &X,
    ...
    Matrix coeffs(size_ - 1, size_-1),Ymatrix(size_-1,1);
    std::vector<std::vector<long double>> res(4,std::vector<long double>(size_));
    ...
    for (size_t i = 1, j = 0; i < size_; i++, j++) {
        ...
        Ymatrix(j,0)=(Y[i + 1] - Y[i]) / h[i] -
                            (Y[i] - Y[i - 1]) / h[i-1];
    }
    coeffs.print(std::cout);
    res[C] = slea.GetResult(coeffs,Ymatrix);
